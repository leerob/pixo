...
fn main() {
    ...
    let width: u32 = args.next().and_then(|s| s.parse().ok()).unwrap_or(512);
    let height: u32 = args.next().and_then(|s| s.parse().ok()).unwrap_or(512);
    ...
    let filtered = filter::apply_filters(&pixels, width, height, bytes_per_pixel, &options);
    ...
    println!("Filter time: {:.3} ms", filter_time.as_secs_f64() * 1000.0);
    println!(
